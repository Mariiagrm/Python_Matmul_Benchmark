...
// Lanza `launcher` con un warmup + `repeat` iteraciones medidas con cudaEvents
// y devuelve el tiempo medio por iteracion en SEGUNDOS. Aloca/libera A,B,C
// internamente: una sola llamada autosuficiente.
template <typename T>
double perf_gemm_swizzle(
    void (*launcher)(T*, T*, T*, int, int, int, int),
    int M, int N, int K, int swizzle_stride, int repeat) {
  size_t sA = (size_t)M * K * sizeof(T);
  size_t sB = (size_t)K * N * sizeof(T);
  size_t sC = (size_t)M * N * sizeof(T);
  ...
  // Warmup
  launcher(dA, dB, dC, M, N, K, swizzle_stride);
