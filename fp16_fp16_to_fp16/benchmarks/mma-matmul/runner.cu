#include <cuda_fp16.h>
#include <math.h>
#include <stdio.h>
#include "mma_kernels.cuh"
#include <cublas_v2.h>
#include <cuda_runtime.h>

// error checking macro
#define cudaCheckErrors(msg) \
    do { \
        cudaError_t __err = cudaGetLastError(); \
        if (__err != cudaSuccess) { \
            fprintf(stderr, "Fatal error: %s (%s at %s:%d)\n", \
                    msg, cudaGetErrorString(__err), \
                    __FILE__, __LINE__); \
            fprintf(stderr, "*** FAILED - ABORTING\n"); \
            exit(1); \
        } \
    } while (0)

#define ceilDiv(x, y) (((x) + (y) - 1) / (y))


#define K_BLOCK 32 
#define REPS 100
#define WARMUP_REPS 10
#define TOTAL_REPS REPS + WARMUP_REPS


// Reference fp16/32 CUDA MATMUL Kernel
__global__ void shmem_matmul(const half *A, const half *B, float *C, int M, int N, int K) {
  __shared__ half As[K_BLOCK][K_BLOCK];
  __shared__ half Bs[K_BLOCK][K_BLOCK];
  int idx = threadIdx.x + blockDim.x*blockIdx.x; 
  int idy = threadIdx.y + blockDim.y*blockIdx.y;

  if ((idx < N) && (idy < M)){
    float temp = 0;
    for (int i = 0; i < K/K_BLOCK; i++) {
      As[threadIdx.y][threadIdx.x] = A[idy*K + i*K_BLOCK + threadIdx.x];
      Bs[threadIdx.y][threadIdx.x] = B[(i*K_BLOCK + threadIdx.y) * N + idx];
      __syncthreads();

      for (int k = 0; k < K_BLOCK; k++)
      	temp += __half2float(As[threadIdx.y][k]) * __half2float(Bs[k][threadIdx.x]); 
      __syncthreads();

    }
    C[idy*N+idx] = temp;
  }
}

float uniform_rand() {
  float u = (float)rand() / (float)RAND_MAX;
  return u;
}

// very hacky approx standard normal generator
float normal_rand() {
  float u = 0; 
  for (int i=0; i<12; i++) 
  {
    u += ((float)rand() / (float)RAND_MAX);
  }
  return (u - 6.) ;
}
void host_transpose(half *src, half *dst, int M, int N) {
  for (int i=0; i < M; i++) {
    for (int j=0; j < N; j++) {
      dst[j*M + i] = src[i*N + j];
    }
  }
}

void init_uniform_half(half **host, half **device, int M, int N) {
  *host = new half[M * N];
  cudaMalloc(device, M * N * sizeof(half));
  for (int i=0; i < M*N; i++) {
    (*host)[i] = __float2half(normal_rand());
  }
  cudaMemcpy(*device, *host, M * N * sizeof(half), cudaMemcpyHostToDevice);
}

bool in_array(int val, int* vals, int N) {
  for (int i=0; i<N; i++) {
    if (vals[i] == val)
      return true;
  }
  return false;
}

void init_zero_float(float** host, float **device, int M, int N) {
  *host = new float[M * N];
  cudaMalloc(device, M * N * sizeof(float));
  for (int i=0; i < M*N; i++) {
    (*host)[i] = 0.;
  }
  cudaMemcpy(*device, *host, M * N * sizeof(float), cudaMemcpyHostToDevice);
}

void init_zero_half(half** host, half **device, int M, int N) {
  *host = new half[M * N];
  cudaMalloc(device, M * N * sizeof(half));
  for (int i=0; i < M*N; i++) {
    (*host)[i] = 0.;
  }
  cudaMemcpy(*device, *host, M * N * sizeof(half), cudaMemcpyHostToDevice);
}



float run_mma_kernel(int kernelNum, int numReps, half *A, half *B, half *B_T, float *C, int M, int N, int K) {
  dim3 mma_block(16, 16);
  dim3 mma_grid;
  int warm_reps = WARMUP_REPS;

    for (int i=0; i<warm_reps; i++) {
    switch (kernelNum) {
     
      case 32:
        mma_grid.x = ceilDiv(M, 128);
        mma_grid.y = ceilDiv(N, 128);
        mma_matmul_3_2<<<mma_grid, mma_block>>>(A, B_T, C, M, N, K);
        break;
      
    }
  }

  cudaDeviceSynchronize();

    // 2. PREPARAR MEDICIÓN
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);

    // 3. MEDIR REPETICIONES REALES
    cudaEventRecord(start);


  for (int i=0; i<numReps; i++) {
    switch (kernelNum) {
      
      case 32:
        mma_grid.x = ceilDiv(M, 128);
        mma_grid.y = ceilDiv(N, 128);
        mma_matmul_3_2<<<mma_grid, mma_block>>>(A, B_T, C, M, N, K);
        break;
      
    }
  }

  cudaEventRecord(stop);
    cudaEventSynchronize(stop);

    // 4. CALCULAR TIEMPO
    float milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop);

    // Limpiar eventos
    cudaEventDestroy(start);
    cudaEventDestroy(stop);

    // Devolver el tiempo promedio por 1 sola ejecución
    return milliseconds / numReps;



}

int main(int argc, char **argv){

  float avg_time_ms = 0.0f;
// Uso: ./programa <kernel> <M> <N> <K>
    if (argc < 2) {
        printf("Uso: %s <kernelNum> [M] [N] [K]\n", argv[0]);
        printf("Ejemplo: %s 31 8192 4096 2048\n", argv[0]);
        printf("Valid Kernel Numbers:  32\n");

        exit(EXIT_FAILURE);
    }
  int kernelNum = atoi(argv[1]);
  int validKernels[1] = {32};
  bool validKernelNum = in_array(kernelNum, validKernels, 11);
  if (not validKernelNum) {
    printf("Kernel Num: %d not recognized. Valid Kernel Nums:  32 \n", kernelNum);
    exit(EXIT_FAILURE);
  }

  // Valores por defecto si no se especifican
    int M = (argc > 2) ? atoi(argv[2]) : 4096;
    int N = (argc > 3) ? atoi(argv[3]) : M; // Si no hay N, N = M
    int K = (argc > 4) ? atoi(argv[4]) : M; // Si no hay K, K = M

printf(" Ejecutando Kernel %d.%d\n", kernelNum/10, kernelNum%10);
printf(" Dimensiones: M=%d, N=%d, K=%d\n", M, N, K);

  printf("Running Kernel %d.%d\n", kernelNum/10, kernelNum%10);
  float *h_C1, *h_C2, *d_C1, *d_C2;
  half  *h_A, *h_B, *h_B_T, *d_A, *d_B, *d_B_T, *h_C3, *d_C3;

  init_uniform_half(&h_A, &d_A, M, K);
  init_uniform_half(&h_B, &d_B, K, N);

  half max_val = 0.;
  half min_val = 0.;
  half avg_val = 0.;
  for (int i=0; i < M*K; i++) {
    avg_val += h_A[i]/(half)(M*K);
    if (h_A[i] < min_val)
      min_val = h_A[i];
    if (h_A[i] > max_val)
      max_val = h_A[i];
  }
  printf("avg A val %f, max A val %f, min A val: %f\n", (float) avg_val, (float) max_val, (float) min_val);

  init_zero_float(&h_C1, &d_C1, M, N);
  init_zero_float(&h_C2, &d_C2, M, N);
  init_zero_half(&h_C3, &d_C3, M, N);
  cudaCheckErrors("cudaMemcpy malloc / H2D failure");

  // Launch reference kernel
  dim3 block(K_BLOCK, K_BLOCK);  
  dim3 grid(ceilDiv(N, block.x), ceilDiv(M, block.y));
  shmem_matmul<<<grid, block>>>(d_A, d_B, d_C1, M, N, K);
  cudaCheckErrors("reference kernel failure");
  cudaMemcpy(h_C1, d_C1, M * N * sizeof(float), cudaMemcpyDeviceToHost);

  // transpose B as Kernel 2.0 on require B to be column-major
  h_B_T = new half[N * K];
  host_transpose(h_B, h_B_T, K, N);
  cudaMalloc(&d_B_T, N * K * sizeof(half));
  cudaMemcpy(d_B_T, h_B_T, N * K * sizeof(half), cudaMemcpyHostToDevice);
  cudaCheckErrors("cudaMemcpy malloc / H2D failure");

 

  // --- CÁLCULO DE TFLOPS ---
    // Cast a double estricto para evitar Integer Overflow con matrices grandes
    double flops = 2.0 * (double)M * (double)N * (double)K;
    double tflops = flops / (avg_time_ms * 1e9);

    // --- IMPRESIÓN DE LA TABLA ---
    printf("\n=================================================================\n");
    printf("%-10s | %-15s | %-15s | %-10s\n", "Kernel", "Dimensiones", "Time_ms", "TFLOPS");
    printf("-----------------------------------------------------------------\n");
    
    // Formatear el string de dimensiones "MxNxK"
    char dims[30];
    sprintf(dims, "%dx%dx%d", M, N, K);
    
    printf("%-10d | %-15s | %-15.4f | %-10.2f\n", kernelNum, dims, avg_time_ms, tflops);
    printf("=================================================================\n\n");

  cudaMemcpy(h_C2, d_C2, M * N *sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(h_C3, d_C3, M * N *sizeof(half), cudaMemcpyDeviceToHost);
  if (kernelNum==1) {
    for (int i=0; i < M * N; i++) {
      h_C2[i] = __half2float(h_C3[i]);
    }
  }

  // check output vs reference kernel
  float diff = 0.;
  float abs_diff = 0.;
  float max_abs_diff = 0.;
  float avg_diff = 0.;
  float avg_abs_diff = 0.;
  float avg_out_val = 0.;

  for (int i = 0; i < M * N; i++) {
    diff = ((h_C1[i]) - h_C2[i]);
    abs_diff = abs(diff);
    avg_abs_diff += abs_diff / (float)(M * N);
    avg_diff += diff / (float)(M * N);
    avg_out_val += h_C2[i] / (float)(M * N);
    if (abs_diff > max_abs_diff)
      max_abs_diff = abs_diff;
  }
  printf("max abs diff %f, avg abs diff %f, avg diff: %f, avg output value; %f\n", max_abs_diff, avg_abs_diff, avg_diff, avg_out_val);
  return 0;
}
