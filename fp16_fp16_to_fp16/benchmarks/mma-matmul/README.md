...
```bash
 ...
 nvidia-smi --lock-gpu-clocks=2520
...
```
 ...
 nvidia-smi --lock-gpu-clocks=2520
 nvidia-smi --lock-memory-clocks=10501 
ncu --set empty --metrics gpu__time_duration.sum -s 10 -c 1 -k regex:'^(?!shmem*)' --clock-control none --print-summary per-gpu ./runner 34 2048
 nvidia-smi --reset-gpu-clocks
 nvidia-smi --reset-memory-clocks
 nvidia-smi -pm DISABLED
```