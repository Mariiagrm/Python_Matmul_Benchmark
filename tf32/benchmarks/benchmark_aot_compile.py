...
# --- 1. CONFIGURACIÓN DE PRECISIÓN Y BACKEND (TF32) ---
device = torch.device("cuda")
torch._logging.set_logs(output_code=True)

# Activamos TF32 a nivel global: entradas FP32 que los Tensor Cores
# procesan internamente como TF32 (mantisa de 10 bits, exp de 8).
torch.backends.cuda.matmul.allow_tf32 = True
torch.backends.cudnn.allow_tf32 = True
torch.set_float32_matmul_precision("high")  # habilita TF32 en matmul FP32
...
def _looks_like_oom(msg: str) -> bool:
    """OOM puede llegar enmascarado desde el runtime AOTI."""
    msg = msg.lower()
    return (
        "out of memory" in msg
        or "aoti_torch_empty_strided" in msg
...
def _bench_one(model, label, M, N, K, dtype, package_dir):
    ...
    try:
        ...
        # Pre-check de memoria: A + B + C + ~25% de holgura para workspace.
        itemsize = torch.empty((), dtype=dtype).element_size()
        needed = (M * K + K * N + M * N) * itemsize
        ...
        package_path = os.path.join(package_dir, f"matmul_tf32_{label}_{M}_{N}_{K}.pt2")
        ep = export(model, (a, b))
        ...
        if _looks_like_oom(str(e)):
            print(f"\n⚠️ OOM (AOTI runtime) en {M}x{N}x{K}. Saltando.")
            torch.cuda.empty_cache()
        else:
...
def run_one_shot(M, N, K, dtype=torch.float32):
    """Ejecuta un unico caso (M,N,K) sin escribir CSV ni dejar .pt2 en disco."""
    print(f"🖥️  GPU: {torch.cuda.get_device_name(0)}")
    print(f"🚀 Modo: AOTI one-shot TF32 — {M}x{N}x{K} (sin guardar resultados)")
...
def run_exhaustive_benchmark(custom_dims=None):
    ...
    print(f"🖥️  GPU: {torch.cuda.get_device_name(0)}")
    print(f"🚀 Modo: AOTI (Ahead-Of-Time Compilation via torch.export) — TF32")
    ...
    for label, M, N, K in tqdm(all_tasks, desc="Benchmarking TF32"):
        res = _bench_one(model, label, M, N, K, torch.float32, "aoti_compiled_models")
        if res is not None:
            results.append(res)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Benchmark AOTI matmul TF32")
    ...
    if args.mnk is not None:
        ...
        if args.no_save:
            ...
            filename = results_dir / "rtx4090_torch_aoti_benchmark_tf32.csv"
            df.to_csv(filename, index=False)
            print(f"\n✅ Benchmark completado. Guardado en {filename}")

        if not df.empty:
            print("\n🔥 Top 5 Rendimiento (TFLOPS) AOTI TF32:")
