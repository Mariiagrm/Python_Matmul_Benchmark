...
kernels_names = {
    "../results/benchmark_mma-matmul.csv": "MMA MatMul with kernel 3.1",
...
for (typ, M, N, K), group in configs:
    for _, row in group.iterrows():
        rows.append({
            ...
            "% 4090 peak FP16/32": round(row["TFLOPS"] / pp_base_fp15_acfp32 * 100, 2),
        })
...
for (typ, M, N, K), group in result.groupby(["Type", "M", "N", "K"], sort=False):
    ...
    print(f"{'='*60}")
    print(group[["Kernel", "Execution Time (ms)", "TFLOP/s", "% 4090 peak FP16/32"]].to_markdown(index=False))
